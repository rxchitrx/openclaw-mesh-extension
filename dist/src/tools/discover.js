            ...
            if (connectTarget) {
                ...
                if (foundPeer && foundPeer.source === "signaling") {
                    ...
                    if (!peerHost || /^\d+\.\d+\.\d+\.\d+$/.test(peerHost) === false) {
                        return {
                            content: [{ type: "text", text: "Invalid address. Use format: IP:port (e.g. 192.168.1.5:18790) or a known peer name." }],
                ...
                if (success) {
                    ...
                    return {
                        content: [{ type: "text", text: `Connected to ${peer.name}. Already approved and connected.` }],
                        details: { ok: true, action: "manual_connect", host: peer.host, port: peer.port },
                    };
                }
                return {
                    content: [{ type: "text", text: `Could not connect to ${peer.name}. Make sure the peer is running OpenClaw with the mesh extension and the port is correct.` }],
