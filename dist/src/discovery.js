...
export function isLocalIPv4Address(host) {
...
function getLocalIP() {
    ...
    if (privateFallback)
        return privateFallback.address;
...
async function scanSubnet(subnet, port, localIPs) {
    const results = [];
    const queue = [];
    for (let i = 1; i <= 254; i++) {
        const ip = `${subnet}.${i}`;
...
export function createDiscovery(config) {
    ...
    let onWebRTCConnectionHook = null;
    return {
        set onWebRTCConnection(hook) {
        ...
        async initiateWebRTCConnection(targetPeerName) {
            if (!signalingClient) {
