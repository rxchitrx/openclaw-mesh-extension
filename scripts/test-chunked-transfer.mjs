...
try {
  ...
  assert.equal(senderNotifications.some((event) => event.type === "file_sent" && event.filePath === relativePath && event.data?.mode === "chunked"), true);

  const tinyPath = "small/tiny.txt";
  const tinyContent = "tiny file";
  senderHashes.set(tinyPath, hashContent(tinyContent));
  await sender.sendFileContent("chunk-receiver", tinyPath, tinyContent, false);
  await waitFor(async () => {
    try {
      return (await fs.readFile(path.join(receiverDir, tinyPath), "utf-8")) === tinyContent;
    } catch {
      return false;
    }
  }, "tiny direct file write");
  assert.equal(senderNotifications.some((event) => event.type === "file_sent" && event.filePath === tinyPath && event.data?.mode === "direct"), true);

  const corruptPath = "large/corrupt.txt";
  receiver.sendToPeer("chunk-sender", {
    type: "file_chunk",
    path: corruptPath,
    transferId: "corrupt-transfer",
    chunkIndex: 0,
    totalChunks: 1,
    chunk: "not-the-hash",
    isBinary: false,
  ...
  await waitFor(
    () => senderNotifications.some((event) => event.type === "sync_failed" && event.filePath === corruptPath && event.data?.reason === "hash_mismatch"),
    "corrupt chunk rejection",
  );
} finally {
  await sender.stop();
  await receiver.stop();
