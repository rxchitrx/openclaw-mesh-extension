    ...
    execute: async (_toolCallId: string, toolParams: { action?: string; peerName?: string; file?: string; force?: boolean }, _signal: any, _onUpdate: any) => {
      ...
      if (!action || action === "manifest") {
        ...
        if (!connections.includes(target)) {
          return {
            content: [{ type: "text" as const, text: `Peer '${target}' is not connected. Connected peers: ${connections.join(", ")}` }],
      ...
      if (!peerName) {
        return {
          content: [{ type: "text" as const, text: `Specify a peer name. Connected peers: ${connections.join(", ")}` }],
      ...
      if (!connections.includes(peerName)) {
        return {
          content: [{ type: "text" as const, text: `Peer '${peerName}' is not connected. Connected peers: ${connections.join(", ")}` }],
          details: { ok: false, error: "not_connected" },
        };
      }
      ...
      if (action === "push") {
        if (!file) {
          return {
            content: [{ type: "text" as const, text: "Specify a file to push. e.g. 'push index.ts to node-123'" }],
        ...
        if (!fileData) {
          return {
            content: [{ type: "text" as const, text: `File '${file}' not found locally.` }],
        ...
        return {
          content: [{ type: "text" as const, text: `Sent '${file}' to '${peerName}' (${fileData.content.length} chars, ${fileData.isBinary ? "binary" : "text"}). It stays pending until the peer confirms file_applied.` }],
      ...
      if (action === "pull") {
        if (!file) {
          return {
            content: [{ type: "text" as const, text: "Specify a file to pull. e.g. 'pull index.ts from node-123'" }],
            details: { ok: false, error: "no_file" },
          };
        }
      ...
      if (action === "push-all") {
        ...
        return {
          content: [{ type: "text" as const, text: `Sent ${sentCount} file(s) to '${peerName}'. Pending changes clear after matching file_applied confirmations.` }],
      ...
      return {
        content: [{ type: "text" as const, text: `Unknown action '${action}'. Use: manifest, push, pull, push-all, pull-all` }],
