...
export type TrackStateReader = () => { currentTrackDir: string | null };

export function ensureTrackedDirectory(syncState: SyncStateService, getTrackState: TrackStateReader) {
  const currentTrackDir = getTrackState().currentTrackDir;
  if (currentTrackDir) {
    return { ok: true as const, currentTrackDir };
  }
...
export function noTrackedDirectoryResponse(guard: ReturnType<typeof ensureTrackedDirectory>) {
