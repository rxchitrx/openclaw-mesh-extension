...
export function createTransport(config: TransportConfig): TransportService {
  ...
  const computeFileHash = async (filePath: string, isBinary: boolean): Promise<string> => {
  ...
  const waitForSocketBackpressure = async (socket: any) => {
  ...
  const sendChunkToPeer = async (peerName: string, message: Record<string, unknown>): Promise<boolean> => {
