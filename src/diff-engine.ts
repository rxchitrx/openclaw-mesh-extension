...
function formatLine(prefix: string, line: string): string {
  return `${prefix}${line}`;
}
...
function buildLcsOps(before: string[], after: string[]): Array<{ type: "context" | "remove" | "add"; line: string; oldLine?: number; newLine?: number }> {
  ...
  const ops: Array<{ type: "context" | "remove" | "add"; line: string; oldLine?: number; newLine?: number }> = [];
  let i = 0;
  let j = 0;
...
function unifiedPatch(path: string, beforeText: string, afterText: string, contextLines: number): string {
  ...
  for (let index = 0; index < ops.length; index += 1) {
    if (ops[index].type !== "context") {
      for (let offset = -contextLines; offset <= contextLines; offset += 1) {
        ...
        if (target >= 0 && target < ops.length) changed.add(target);
      }
    }
  }
  ...
  for (const chunk of chunks) {
    ...
    for (const index of chunk) {
      const op = ops[index];
      if (op.type === "context") lines.push(formatLine(" ", op.line));
      if (op.type === "remove") lines.push(formatLine("-", op.line));
      if (op.type === "add") lines.push(formatLine("+", op.line));
    }
  }
...
export function createDiffPreview(options: DiffPreviewOptions): DiffPreview {
  ...
  const kind: DiffPreview["kind"] = !localFile && remoteFile ? "deleted" : localFile && !remoteFile ? "added" : localHash === remoteHash ? "unchanged" : "modified";
...
export function createPatchPayload(path: string, oldText: string, newText: string, parentHash: string, targetHash: string, contextLines = 3) {
