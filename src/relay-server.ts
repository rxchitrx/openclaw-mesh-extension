...
function send(socket: any, frame: Record<string, unknown>) {
  if (socket.readyState === 1) {
    socket.send(JSON.stringify(frame));
  }
}

export async function startMeshRelayServer(options: MeshRelayServerOptions = {}): Promise<MeshRelayServerHandle> {
  const { WebSocketServer } = await import("ws");
  const logger = options.logger ?? console;
  const server = createServer();
  const wss = new WebSocketServer({ server, maxPayload: MAX_RAW_MESSAGE_BYTES + 4096 });
  const peersBySocket = new Map<any, RegisteredPeer>();
  const rooms = new Map<string, Map<string, RegisteredPeer>>();

  const unregister = (socket: any) => {
  ...
  const register = (socket: any, message: Record<string, unknown>) => {
    ...
    for (const other of room.values()) {
      if (other.nodeName === nodeName) continue;
      send(other.socket, {
        type: "peer_online",
        name: nodeName,
  ...
  const forward = (socket: any, message: Record<string, unknown>) => {
  ...
  wss.on("connection", (socket: any) => {
    ...
    socket.on("error", (err: Error) => logger.warn?.(`Relay socket error: ${err.message}`));
  });
